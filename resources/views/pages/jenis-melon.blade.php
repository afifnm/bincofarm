...
@section('content')
    ...
    <div class="flex gap-1.5 mb-5">
        <a href="{{ route('greenhouse') }}"
           ...
           onmouseout="this.style.background='var(--color-surface)'">Greenhouse</a>
        <a href="{{ route('jenis-melon') }}"
           class="px-4 py-2 rounded-xl text-sm font-medium transition-colors"
           style="background:var(--color-primary);color:#fff;">Jenis Melon</a>
    </div>
    ...
    <div class="mb-4">
        <div class="relative max-w-sm">
            ...
            <input type="text" id="search" placeholder="Cari jenis melon..." class="form-input pl-9"/>
        </div>
    </div>
    ...
    <div id="formModal" class="hidden fixed inset-0 z-[9999] flex items-center justify-center p-4" style="background:rgba(0,0,0,0.5);">
        ...
        <div class="relative w-full max-w-lg rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto"
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b" style="border-color:var(--color-border);">
                <h3 id="formModalTitle" class="text-sm font-semibold" style="color:var(--color-text);">Tambah Jenis Melon</h3>
            ...
            <div class="px-5 py-5">
                <form id="mainForm" onsubmit="save(event)" class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="fNama" required class="form-input" placeholder="Contoh: Honey Dew"/>
                    </div>
                    <div>
                        <label class="form-label">Deskripsi</label>
                        <textarea id="fDeskripsi" class="form-input" rows="3" placeholder="Deskripsi varietas (opsional)"></textarea>
                    </div>
                    <div class="flex items-center gap-2 py-1">
                        <input type="checkbox" id="fIsActive" class="w-4 h-4 rounded accent-emerald-600 cursor-pointer"/>
                        <label for="fIsActive" class="text-sm cursor-pointer" style="color:var(--color-text);">Aktif</label>
                    </div>
    ...
    <div id="deleteModal" class="hidden fixed inset-0 z-[9999] flex items-center justify-center p-4" style="background:rgba(0,0,0,0.5);">
        ...
        <div class="relative w-full max-w-md rounded-2xl shadow-2xl" style="background:var(--color-surface);border:1px solid var(--color-border);" onclick="event.stopPropagation()">
            <div class="px-5 py-5">
                <p class="text-sm font-medium mb-1 text-center" style="color:var(--color-text);">Hapus <strong id="deleteTargetName"></strong>?</p>
...
function render() {
    ...
    if (!app.items.length) {
        tbody.innerHTML = `<tr><td colspan="5" class="tbl-cell text-center py-12" style="color:var(--color-text-muted);"><p class="text-sm font-medium">Belum ada jenis melon</p></td></tr>`;
        document.getElementById('pagination').classList.add('hidden');
        return;
    }
    tbody.innerHTML = app.items.map((jm, idx) => `
        <tr class="tbl-row">
            <td class="tbl-cell text-center text-xs" style="color:var(--color-text-muted);">${(app.meta.from||1)+idx}</td>
            <td class="tbl-cell font-semibold" style="color:var(--color-text);">${escHtml(jm.nama)}</td>
            <td class="tbl-cell hidden md:table-cell text-xs" style="color:var(--color-text-muted);">${escHtml(jm.deskripsi||'—')}</td>
            <td class="tbl-cell text-center"><span class="badge ${jm.is_active?'badge-success':'badge-neutral'}">${jm.is_active?'Aktif':'Nonaktif'}</span></td>
            <td class="tbl-cell">
                <div class="flex items-center justify-center gap-1">
                    <button onclick="openEditById(${jm.id})" title="Edit" class="w-8 h-8 flex items-center justify-center rounded-lg transition-colors" style="color:var(--color-text-muted);" onmouseover="this.style.background='var(--color-bg)';this.style.color='var(--color-primary)'" onmouseout="this.style.background='';this.style.color='var(--color-text-muted)'">
                    ...
                    <button onclick="openDeleteById(${jm.id})" title="Hapus" class="w-8 h-8 flex items-center justify-center rounded-lg transition-colors" style="color:var(--color-text-muted);" onmouseover="this.style.background='#FEE2E2';this.style.color='#B91C1C'" onmouseout="this.style.background='';this.style.color='var(--color-text-muted)'">
...
function renderPagination() {
    ...
    pag.innerHTML = `
        <p class="text-xs" style="color:var(--color-text-muted);">Menampilkan ${m.from}–${m.to} dari ${m.total} data</p>
...
function openCreate() {
    app.editId = null;
    document.getElementById('formModalTitle').textContent = 'Tambah Jenis Melon';
...
function openEditById(id) {
    const jm = app.items.find(i => i.id === id);
    if (!jm) return;
    app.editId = jm.id;
    document.getElementById('formModalTitle').textContent = 'Edit Jenis Melon';
    document.getElementById('fNama').value = jm.nama;
    document.getElementById('fDeskripsi').value = jm.deskripsi || '';
    document.getElementById('fIsActive').checked = !!jm.is_active;
    openModal('formModal');
}
...
function openDeleteById(id) {
    ...
    if (!jm) return;
    app.deleteTarget = jm;
    document.getElementById('deleteTargetName').textContent = jm.nama;
...
async function save(e) {
    ...
    const url    = app.editId ? `/api/jenis-melon/${app.editId}` : '/api/jenis-melon';
    const method = app.editId ? 'PUT' : 'POST';
    const res    = await apiFetch(url, { method, body: JSON.stringify(form) });
    const data   = await res.json();
    if (res.ok) {
        toast(app.editId ? 'Jenis melon diperbarui.' : 'Jenis melon ditambah.', 'success');
...
async function doDelete() {
    ...
    try {
        const res  = await apiFetch(`/api/jenis-melon/${app.deleteTarget.id}`, { method: 'DELETE' });
        if (!res) return;
        const data = await res.json();
        if (res.ok) {
